...
public class WaiterAgent extends Agent implements Waiter{
   ...
   //State variables for Waiter
	...
	{NOT_ENOUGH_MONEY,NEED_SEATED,READY_TO_ORDER,ORDER_PENDING,REORDER,ORDER_READY,IS_DONE,NO_ACTION,NOTIFY_CASHIER,DELIVER_BILL,BILL_PAID,CUSTOMERLEAVE,NOCUSTOMERS};
    ...
    public CostcoMarketAgent costco;
	...
	public Order(WaiterAgent w,int tableNum, String choice){
	    this.choice = choice;
    ...
    public void msgHereIsMyChoice(final CustomerAgent customer, final String choice){
	print("Taking order for " + customer.getName() + " (3000 milliseconds)");
	timer.schedule(new TimerTask()
	{
		public void run()
		{		    
		    doneOrdering(customer,choice);
		}
		}, 3000);
    }
    
    public void doneOrdering(CustomerAgent customer, String choice)
    {
    	s.release();
    	print("waiter released permit");
    ...
    /*Cook sends this msg to the waiter when there is no more of that item*/
    public void msgSorryOutOfItem(int tableNum, Food f)
    {
    	print("Waiter is notified customers order is sold out.");
    	for(MyCustomer c:customers)
    	{
    ...
    {
    	print("Waiter notified customer wants bill");
    	for(MyCustomer c:customers)
    	{
    		if(c.cmr.equals(customer))
    		{
    			c.state = CustomerState.NOTIFY_CASHIER;
    			stateChanged();
    			return;
    		}
    	}
    }
    //cashier gives bill to waiter
    public void msgHereIsBill(int tableNum, double total, int billNumber)
    {
    	print("Waiter received bill from cashier");
    	for(MyCustomer c: customers)
    	{
    		if(c.tableNum == tableNum)
    		{
    			c.total = total;
    			c.billNumber = billNumber;
    			c.state = CustomerState.DELIVER_BILL;
    			stateChanged();
    			return;
    		}
    	}
    ...
    {
    	print("Customer told waiter he finished paying");
    	for(MyCustomer c: customers)
    	{
    		if(c.billNumber == billNumber)
    		{
    			c.state = CustomerState.BILL_PAID;
    			stateChanged();
    			return;
    		}
    	}
    ...
    {
    	print("customer told waiter he DOES NOT HAVE ENOUGH MONEY!");
    	for(MyCustomer c: customers)
    	{
    		if(c.billNumber == billNumber)
    		{
    			c.state = CustomerState.NOT_ENOUGH_MONEY;
    			stateChanged();
    			return;
    		}
    	}
    ...
    {
    	print("Waiter notified that customer can leave");
    	for(MyCustomer c: customers)
    	{
    		if(c.tableNum == tableNum)
    		{
    			c.state = CustomerState.CUSTOMERLEAVE;
    			stateChanged();
    			return;
    		}
    	}
    ...
    {
    	print("Waiter notified that customer needs to leave!");
    	for(MyCustomer c: customers)
    	{
    		if(c.tableNum == tableNum)
    		{
    ...
    {
    	//onBreak = true;
    	print("break for 2000 milliseconds");
		timer.schedule(new TimerTask()
		{
		    public void run()
		    {		 
    ...
    public void breakDone(WaiterAgent waiter)
    {
    	//onBreak = false;
    	print("break done");
    	host.msgBreakDone(waiter);
    }
    
    public void msgCantGoOnBreak()
    {
    	onBreak = false;
    	print("can not go on break");
    }
	...
	//onBreak = state;
   print("customers size " + customers.size());
	if(customers.size() == 0)
	{
		askHostForBreak();
		stateChanged();
	...
	if(!customers.isEmpty() && s.availablePermits() == 1){
	    ...
	    for(MyCustomer c:customers)
	    {
			if(c.state == CustomerState.NOTIFY_CASHIER) 
			{
			    getBillFromCashier(c);
			    return true;
			}
	    }
	    for(MyCustomer c:customers)
	    {
			if(c.state == CustomerState.DELIVER_BILL) 
			{
	    ...
	    for(MyCustomer c:customers)
	    {
			if(c.state == CustomerState.BILL_PAID) 
			{
			    customerPaidBill(c);
			    return true;
			}
	    ...
	    for(MyCustomer c:customers)
	    {
			if(c.state == CustomerState.NOT_ENOUGH_MONEY) 
			{
			    customerNotEnoughMoney(c);
			    return true;
			}
	    }
	    ...
	    for(MyCustomer c:customers)
	    {
			if(c.state == CustomerState.CUSTOMERLEAVE) 
			{
	...
	{
		if(customers.size() == 0)
			print("no customers, will notify cook to check inventory");
    ...
    try {
		s.acquire();
		print("waiter took permit");
	} catch (InterruptedException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
	//customer.cmr.s.release();
    ...
    /*Waiter notifies customer that we ran out of his first order and waiter asks to reorder */
    protected void retakeOrder(MyCustomer customer) 
    {
    	print("Sorry, we are out of the choice you wanted, please reorder.");
		DoTakeOrder(customer); //animation
		customer.state = CustomerState.NO_ACTION;
	    try {
			s.acquire();
	...
	//Order o = new Order(waiter,customer.tableNum,customer.choice);
    ...
    protected void getBillFromCashier(MyCustomer customer)
    {
    	print("Waiter telling cashier to get bill");
    	customer.state = CustomerState.NO_ACTION;
    	cashier.msgCustomerWantsBill(this,customer.choice,customer.tableNum, new Menu());
    	stateChanged();
    }
    
    protected void giveBillToCustomer(MyCustomer customer)
    {
    	print("Waiter giving bill to customer");
    	customer.state = CustomerState.NO_ACTION;
    	customer.cmr.msgHereIsYourBill(customer.total, customer.billNumber);
    	stateChanged();
    }
    
    protected void customerPaidBill(MyCustomer customer)
    {
    	print("Waiter telling cashier customer paid.");
    	customer.state = CustomerState.NO_ACTION;
    	cashier.msgCustomerPaid(customer.billNumber);
    	stateChanged();
    }
    
    protected void customerNotEnoughMoney(MyCustomer customer)
    {
    	print("Waiter telling cashier customer doesn't have enough money.");
    	customer.state = CustomerState.NO_ACTION;
    	cashier.msgCustomerNotEnough(customer.billNumber);
    	stateChanged();
    }
    
    protected void tellCustomerToLeave(MyCustomer customer)
    {
    	print("waiter telling customer can leave");
    	customer.state = CustomerState.NO_ACTION;
    	customer.cmr.msgYouCanLeave();
    	stateChanged();
    }
    ...
    //HAS A STATE CHANGE BUT DON'T KNOW IF NECESSARY
    //HAS A STATE CHANGE BUT DON'T KNOW IF NECESSARY
    //HAS A STATE CHANGE BUT DON'T KNOW IF NECESSARY
    //HAS A STATE CHANGE BUT DON'T KNOW IF NECESSARY
    //HAS A STATE CHANGE BUT DON'T KNOW IF NECESSARY
  ...
  /* private void tellCookToCheckInventory()
    {
	   //customer.state = CustomerState.NO_ACTION;
    	print("waiter telling cook to check inventory since ther are no customers");
    ...
    void DoTakeOrder(MyCustomer customer){
        /*try {
    		s.acquire();
    		print("waiter takes permit");
    	} catch (InterruptedException e) {
    		// TODO Auto-generated catch block
    		e.printStackTrace();
    		print("waiter did not get permit");
    	}*/

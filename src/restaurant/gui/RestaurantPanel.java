...
import restaurant.*;
//import restaurant.CookAgent.ProducerConsumerMonitor;
import astar.*;
...
public class RestaurantPanel extends JPanel {
    ...
    private ProducerConsumerMonitor monitor = new ProducerConsumerMonitor();
    private HostAgent host = new HostAgent("Prof. W.", nTables);
    private CookAgent cook = new CookAgent("W. Puck", restaurant, monitor);
    private CashierAgent cashier = new CashierAgent("Edward The Great");
    private CostcoMarketAgent costco = new CostcoMarketAgent("Costco");
    private RalphsMarketAgent ralphs = new RalphsMarketAgent("Ralphs");
    private Vector<CustomerAgent> customers = new Vector<CustomerAgent>();
    private Vector<WaiterAgent> waiters = new Vector<WaiterAgent>();
    private Vector<ProducerAgent> producers = new Vector<ProducerAgent>();

    private JPanel restLabel = new JPanel();
    private ListPanel customerPanel = new ListPanel(this, "Customers");
    private ListPanel waiterPanel = new ListPanel(this, "Waiters");
    private ListPanel producerPanel = new ListPanel(this, "Producers");
    private CookPanel cookPanel = new CookPanel(this,"Cook");
    private CostcoMarketPanel costcoPanel = new CostcoMarketPanel(this,"Costco Market");
    private RalphsMarketPanel ralphsPanel = new RalphsMarketPanel(this,"Ralphs Market");
	...
	group.add(producerPanel);
	group.add(customerPanel);
	group.add(cookPanel);
	group.add(costcoPanel);
	group.add(ralphsPanel);
	...
	else if(type.equals("Producers"))
	{
	    ...
	    {
	    	ProducerAgent temp = producers.get(i);
	    	if(temp.getName() == name)
	    	{
	    		gui.updateInfoPanel(temp);
	...
	{
			System.out.println("setting info for producers");
		    AStarTraversal aStarTraversal = new AStarTraversal(grid);
		    ProducerAgent p = new ProducerAgent(name, aStarTraversal, restaurant, tables, monitor);
		    p.setHost(host);
		    p.setCook(cook);
		   ...
		   // w.setMarket(costco);
		    host.setWaiter(p);
		    producers.add(p);
		    //System.out.println("producers size: " + producers.size());
		    p.startThread();
	}
    }	
    
    public void addPersonThree(String type, String name, double amount){
    ...
    //gui sets food for the restaurant or markets
    public void addFood(String type, String name, int amount)
    {
 	   if(type.equals("Cook"))
 		{
 		  cook.setMarket(costco);
 		  cook.setMarket2(ralphs);
 		  cook.setFood(amount, name);
 		  cook.startThread();
 		   
 				
 		}
 	   else if(type.equals("Costco Market"))
 	   {
 		 // CostcoMarketAgent market = new CostcoMarketAgent(name);
 		   costco.setCashier(cashier);
 		   costco.setCook(cook);
 		   costco.setFood(amount, name);
 		   costco.startThread();

 	   }
 	   else if(type.equals("Ralphs Market"))
 	   {
 		   ralphs.setCashier(cashier);
 		   ralphs.setCook(cook);
 		   ralphs.setFood(amount, name);
 		   ralphs.startThread();
